...
export class RoomService {
  ...
  addRoom(room: Room, id: number): Observable<Room> {
    return this.http.post<Room>(`${this.baseUrl}/addRoom/` + id, room);
  }
  ...
  assignMembershiptoRoom(
    idRoom: number,
    idFmembership: number
  ): Observable<any> {
    return this.http.put(
      `${this.baseUrl}/assignMembershiptoRoom/${idRoom}/${idFmembership}`,
      null
    );
  }
}
